    ...
    def target():
        try:
            ...
            if process.returncode == 0:
                ...
                log(f"⚠️ 命令执行失败，错误码: {process.returncode}")
            if callback:
    ...
    def info_task():
        try:
            ...
            if result.returncode != 0:
                log(f"⚠️ ffprobe 执行失败: {result.stderr}")
                return
            ...
            if duration:
                ...
                seconds = sec % 60
                if hours > 0:
                    lines.append(f"⏱️ 时长: {hours:02d}:{minutes:02d}:{seconds:06.3f}")
                else:
                    lines.append(f"⏱️ 时长: {minutes:02d}:{seconds:06.3f}")
            bitrate = fmt.get('bit_rate')
            if bitrate:
                lines.append(f"🎛️ 总比特率: {int(bitrate)//1000} kbps")
...
class App:
    def __init__(self, root):
        ...
        ttk.Button(tab5, text="▶️ 播放", command=lambda: do_preview(entry5_in, entry5_start)).grid(row=2, column=1, pady=15)
